...
def load_to_database():
    ...
    with open('/home/paul/data-pipeline/processed_data/processed_destination_data.csv') as file:
        cur.copy_expert("COPY destinations(id,destination,country,popular_season) FROM STDIN WITH (FORMAT 'csv', HEADER false)",file)
    with open('/home/paul/data-pipeline/processed_data/processed_customer_data.csv') as file:
        cur.copy_expert("COPY customers(id,first_name,last_name,email,phone) FROM STDIN WITH CSV (FORMAT 'csv', HEADER false)",file)
